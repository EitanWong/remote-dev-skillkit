...
const PackageSchemaVersion = "rdev.coder-adapter-package.v1"
const VerificationSchemaVersion = "rdev.coder-adapter-package-verification.v1"
const AcceptanceEvidencePlanSchemaVersion = "rdev.coder-adapter-acceptance-evidence-plan.v1"

// AdapterKind is the canonical identifier for this adapter.
const AdapterKind = "coder"
...
// Build generates a Coder adapter package in opts.OutDir.
func Build(opts Options) (Package, error) {
	...
	if name == "" {
		name = "rdev-coder-workspace-adapter"
	}
	pkg := Package{
		...
		ExternalMutation: true, // workspace start/stop mutates remote Coder state
		ProductionClaim:  "coder-workspace-adapter-package-surface-only",
		Helper: Helper{
			Tool:               "coder",
			Scope:              "user",
			SupportedPlatforms: []string{"darwin/amd64", "darwin/arm64", "linux/amd64", "linux/arm64", "windows/amd64", "windows/arm64"},
			VerifyCommand:      "coder version",
			RuntimeStatus:      "authenticated-session-required",
		},
		ConnectionPathID: "existing-coder-workspace",
		EvidencePlanPath: "acceptance-evidence-plan.json",
		EvidenceRequired: []string{
			"coder adapter package verification",
			"coder CLI version and authentication status",
			"workspace start or status evidence from coder workspace show",
		...
		AuthorizationRequired: []string{
			...
			"storing Coder API tokens or session credentials outside the workspace",
		},
		AgentRules: []string{
			"Use the coder CLI with --url ${RDEV_CODER_URL} and --token ${RDEV_CODER_TOKEN}; do not hardcode URLs or tokens.",
	...
	files := []struct {
		...
		{"CODER_ADAPTER.md", "documentation", []byte(renderReadme(pkg))},
	...
	if err := os.WriteFile(filepath.Join(outDir, "coder-adapter.json"), append(manifest, '\n'), 0o644); err != nil {
		return Package{}, err
	}
	return pkg, nil
}

// Verify checks an existing Coder adapter package for correctness.
func Verify(path string) (Verification, error) {
	...
	v := Verification{
		...
		PackagePath:   "coder-adapter.json",
		PackageDir:    ".",
	}
	...
	add("manifest_exists", err == nil, "coder-adapter.json")
	if err != nil {
...
func acceptanceEvidencePlan(pkg Package, generatedAt time.Time) AcceptanceEvidencePlan {
	return AcceptanceEvidencePlan{
		...
		EvidenceFiles: []EvidencePlanFile{
			{Name: "coder-version", Path: "coder-version.txt", Kind: "transcript", Required: true, Description: "Output of coder version confirming CLI is present and authenticated."},
			{Name: "workspace-status", Path: "workspace-status.json", Kind: "json", Required: true, Description: "Output of coder workspace show (redacted — no org IDs or tokens)."},
			{Name: "host-registration", Path: "host-registration.json", Kind: "json", Required: true, Description: "Host registration evidence from rdev host serve inside the workspace."},
			{Name: "workspace-stop", Path: "workspace-stop.txt", Kind: "transcript", Required: true, Description: "Evidence of workspace stop or cleanup confirming no persistence."},
			{Name: "audit", Path: "audit.jsonl", Kind: "transcript", Required: true, Description: "Redacted audit JSONL covering workspace start, host registration, and stop."},
		},
		AgentRules: []string{
...
func renderReadme(pkg Package) string {
	return fmt.Sprintf(`# Remote Dev Skillkit Coder Workspace Adapter Package

Schema: %s
Name: %s
Adapter: %s

This package gives Agents a standard Coder workspace helper surface. It
contains no real Coder URLs, tokens, workspace names, or organisation IDs.
...
- RDEV_CODER_URL: the base URL of the Coder deployment.
- RDEV_CODER_TOKEN: a session token with workspace start/stop scope.
...
func renderEnvTemplate() string {
	return `# Base URL of the Coder deployment.
RDEV_CODER_URL=

# Session token with workspace start/stop scope.
# Keep this outside version control and shared package files.
RDEV_CODER_TOKEN=

# Workspace name to start.
RDEV_CODER_WORKSPACE=
`
}
...
func resolveManifest(path string) (string, string, error) {
	...
	if info.IsDir() {
		return filepath.Join(abs, "coder-adapter.json"), abs, nil
	}
...
func failureActions() []string {
	return []string{
		"Regenerate the Coder adapter package in a clean output directory.",
		"Keep real Coder URLs, tokens, workspace names, and organisation IDs outside package files.",
	}
}

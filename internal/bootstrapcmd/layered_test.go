...
func TestLayeredRunCLIRequiresTemporaryWindowsAMD64Contract(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "wrong platform", args: []string{"--manifest-url", "https://gateway.test/layered-assets.json", "--root-public-key", root, "--expected-release-version", fixture.manifest.Version, "--platform", "windows-amd64", "--cache-dir", t.TempDir(), "--mode", "temporary"}, want: "supported --platform"},

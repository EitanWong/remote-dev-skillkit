...
func PolicyTemplate(capability, targetOS string) map[string]any {
	...
	case "file.delete", "file.remove":
		...
		caps = []string{"file.transfer.write", "fs.write.scoped"}
		writeScope = []string{"."}
		authorizations = []string{"file.delete"}
	...
	case "unattended.access":
		...
		authorizations = []string{"unattended.access"}
	...
	if adapter == "powershell" {
		...
		if len(authorizations) > 0 {
			policy["authorizations_required"] = authorizations
		}
		if adapter == "desktop" {
			switch action {
			case "input.keyboard":
	...
	return map[string]any{
		"schema_version": "rdev.task-policy-template.v1",
		"capability":     capability,
		"target_os":      targetOS,
		"adapter":        adapter,
		"intent":         intent,
		"policy":         policy,
		"session_task_example": map[string]any{
			"mcp_tool": "rdev.sessions.task",
			"arguments": map[string]any{
				...
				"capabilities":       caps,
				"payload":            policy,
				"idempotency_key":    "task:<stable-intent-hash>",
			},
		},
		"agent_rule": "Use this policy object as the reviewed task payload for rdev.sessions.task; do not search source code or hand-roll policy JSON.",
	}
}

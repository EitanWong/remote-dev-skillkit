...
func TestKeyctlProtectedStoreSavesAndLoadsTrustBundle(t *testing.T) {
	backend := &trustMemoryKeyctlBackend{items: map[string][]byte{}}
	restore := protectedstore.SetKeyctlBackendForTest(backend)
	defer restore()

	now := time.Date(2026, 6, 29, 12, 0, 0, 0, time.UTC)
	publicKey, privateKey := testKeyPair(t)
	store, err := OpenStore("keyctl:remote-dev-skillkit/managed-linux-trust")
	if err != nil {
		t.Fatal(err)
	}
	bundle := signedBundle(t, model.SignedTrustBundleSpec{
	...
	if !ok {
		t.Fatal("expected keyctl protected trust bundle")
	}
	if loaded.Sequence != bundle.Sequence || loaded.BundleID != bundle.BundleID {
		t.Fatalf("loaded wrong keyctl protected bundle: %#v", loaded)
	}
}
...
type trustMemoryKeyctlBackend struct {
	items map[string][]byte
}

func (b *trustMemoryKeyctlBackend) Load(service, account string) ([]byte, bool, error) {
	content, ok := b.items[service+"/"+account]
	if !ok {
		return nil, false, nil
	}
...
func (b *trustMemoryKeyctlBackend) Save(service, account string, content []byte) error {
	b.items[service+"/"+account] = append([]byte(nil), content...)
	return nil
}

...
func installDoc(gatewayURL string) string {
	...
	return fmt.Sprintf(strings.Join([]string{
		...
		"%s",
		"",
		"For personal-computer Agent installs, this value may be absent. Local MCP stdio with `rdev mcp serve` is enough to install the skills and expose tools to the current agent runtime. Choose a gateway only when a remote-host workflow needs local dev, LAN, hosted, relay/mesh/VPN, or SSH-tunnel connectivity.",
		"",
		...
		"4. Keep `safe-remote-support`, `host-triage`, `remote-vibe-coding`, and `remote-session-review` installed together.",
		"5. Run a read-only host triage task before any repair or coding task.",
		"6. Export an evidence bundle before declaring remote work complete.",
		"",
		"## Safety Defaults",
		"",
		"- Temporary hosts should use attended foreground mode.",
		"- Agents must request typed tasks, not raw machine access.",
		"- Package install, elevation, GUI control, service mutation, push, merge, deploy, publish, paid action, and credential changes require authorization.",
...
func frameworkDoc(name, gatewayURL string) string {
	...
	return fmt.Sprintf(strings.Join([]string{
		...
		"   ```",
		"",
		"5. For personal-computer installs, stop here and use local MCP stdio. For hosted gateways, configure the framework's MCP HTTP or API client to use:",
		"",
		"   ```text",
		"   %s",
		"   ```",
		...
		"7. Ask the agent to use `host-triage` first, then `safe-remote-support` or `remote-vibe-coding`, and finally `remote-session-review`.",
		"",
		"## Required Review Step",
		"",
		"Before the agent claims success, it should call the artifact/evidence tools and review the session evidence manifest and session artifacts.",
		"",
	}, "\n"), name, name, gateway)
}

...
func installDoc(gatewayURL string) string {
	...
	return fmt.Sprintf(strings.Join([]string{
		...
		"",
		"1. Copy the folders under `skills/` into your agent runtime's skill or instruction directory.",
		"2. Configure the agent's MCP client to call `rdev mcp serve` for local stdio mode, or point it at your hosted rdev MCP HTTP endpoint.",
		"3. Keep `safe-remote-support`, `host-triage`, `remote-vibe-coding`, and `remote-job-review` installed together.",
		"4. Run a read-only host triage job before any repair or coding job.",
		"5. Export an evidence bundle before declaring remote work complete.",
		"",
		"## Safety Defaults",
		"",
...
func frameworkDoc(name, gatewayURL string) string {
	...
	return fmt.Sprintf(strings.Join([]string{
		...
		"2. Copy `skills/*` into the framework's skill/instructions location.",
		"3. Configure MCP stdio with:",
		"",
		"   ```bash",
		"   rdev mcp serve",
		"   ```",
		"",
		"4. For hosted gateways, configure the framework's MCP HTTP or API client to use:",
		"",
		"   ```text",
		"   %s",
		"   ```",
		"",
		"5. Ask the agent to use `host-triage` first, then `safe-remote-support` or `remote-vibe-coding`, and finally `remote-job-review`.",
		"",

...
func PlanInstall(opts InstallPlanOptions) (InstallPlan, error) {
	...
	for _, framework := range frameworks {
		...
		plans = append(plans, FrameworkInstallPlan{
			...
			ReviewNotes: []string{
				...
				"If gateway, session join code, root key, release URL, checksum, framework path, workspace, adapter, or authorization policy is unclear, ask instead of inventing a value.",
	...
	plan := InstallPlan{
		...
		RecommendedNextSteps: []string{
			...
			"Ask a short follow-up question whenever gateway, session join code, release, framework path, workspace, adapter, or authorization policy cannot be discovered safely.",
...
func shellInstallScript(bundleDir, rdevCommand string, spec frameworkInstallSpec, skills []string) string {
	...
	return strings.Join([]string{
		...
		`printf '%s\n' "If gateway URL, session join code, root key, release URL, checksum, framework install path, workspace root, adapter choice, tunnel/mesh authorization, or authorization policy is unclear, ask instead of inventing a value."`,
...
func powerShellInstallScript(bundleDir, rdevCommand string, spec frameworkInstallSpec, skills []string) string {
	...
	return strings.Join([]string{
		...
		`Write-Output "If gateway URL, session join code, root key, release URL, checksum, framework install path, workspace root, adapter choice, tunnel/mesh authorization, or authorization policy is unclear, ask instead of inventing a value."`,
...
func installCommandsDoc(bundleDir, rdevCommand string, plans []FrameworkInstallPlan) string {
	lines := []string{
		...
		"If direct reachability is blocked, prefer existing or open-source/free tunnel/mesh options such as frp, Chisel, headscale, or WireGuard before paid relays.",
		"",
		"If gateway URL, session join code, root key, release URL, checksum, framework install path, workspace root, adapter choice, tunnel/mesh authorization, or authorization policy is unclear, ask a short follow-up question instead of inventing a value.",
		"",
		"Examples such as `https://api.example.com/v1`, `/Users/example`, `/home/example`, and `C:\\Users\\Alice` are placeholders.",
		"",
		"## Framework Scripts",

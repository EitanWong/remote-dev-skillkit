...
func shellInstallScript(bundleDir, rdevCommand string, spec frameworkInstallSpec, skills []string) string {
	...
	return strings.Join([]string{
		...
		`printf '%s\n' "Adaptive Configuration Contract: before using these skills, probe: rdev doctor; rdev mcp tools; OS/shell; service manager; gateway; connection modes; workspace; adapters; framework install path; permissions."`,
...
func powerShellInstallScript(bundleDir, rdevCommand string, spec frameworkInstallSpec, skills []string) string {
	...
	return strings.Join([]string{
		...
		`Write-Output "Adaptive Configuration Contract: before using these skills, probe: rdev doctor; rdev mcp tools; OS/shell; service manager; gateway; connection modes; workspace; adapters; framework install path; permissions."`,
...
func installCommandsDoc(bundleDir, rdevCommand string, plans []FrameworkInstallPlan) string {
	lines := []string{
		...
		"",
		"Before using the installed skills, probe the runtime with `rdev doctor`, `rdev mcp tools`, OS/shell checks, service-manager checks, gateway configuration checks, connection modes checks, workspace checks, adapter detection, framework install path checks, and permission checks.",
		"",
		"For personal-computer installs, local MCP stdio with `rdev mcp serve` is enough. Hosted gateway URLs are optional until a remote-host workflow needs local dev, LAN, hosted, relay/mesh/VPN, or SSH-tunnel connectivity.",
		"",

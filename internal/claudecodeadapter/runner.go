...
const ResultSchemaVersion = "rdev.claude-code-result.v1"
...
type Result struct {
	...
	ClaudeCodeCommand   CommandResult   `json:"claude_code_command"`
...
func ExecuteContext(parent context.Context, spec Spec) (Result, error) {
	...
	if prompt == "" {
		return Result{}, fmt.Errorf("claude-code prompt is required")
	}
	...
	argv := claudeCodeArgv(prompt, spec)
	claudeCodeResult := runCommandWithEnvironment(ctx, workspaceRoot, argv, maxOutputBytes, spec.Environment)
	result := Result{
		Adapter:           "claude-code",
		WorkspaceRoot:     workspaceRoot,
		Prompt:            prompt,
		ClaudeCodeCommand: claudeCodeResult,
	...
	if !claudeCodeResult.Canceled && !claudeCodeResult.TimedOut {
	...
	result.EndedAt = ended.Format(time.RFC3339Nano)
	result.DurationMillis = ended.Sub(started).Milliseconds()
	if claudeCodeResult.Canceled {
		return result, fmt.Errorf("claude code command canceled")
	}
	if claudeCodeResult.TimedOut {
		return result, fmt.Errorf("claude code command timed out after %ds", maxDuration)
	}
	if claudeCodeResult.ExitCode != 0 {
		return result, fmt.Errorf("claude code command exited with status %d", claudeCodeResult.ExitCode)
	}
...
func claudeCodeArgv(prompt string, spec Spec) []string {
	command := strings.TrimSpace(spec.ClaudeCodeCommand)
	if command == "" {
		command = "claude"
	}
	args := append([]string(nil), spec.ClaudeCodeArgs...)
	if len(args) == 0 {
...
func (r Result) Artifact() ResultArtifact {
	...
	return ResultArtifact{
		...
		ClaudeCodeCommand:   redactCommand(redactor, r.ClaudeCodeCommand),

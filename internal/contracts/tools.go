...
func Tools() []Tool {
	return []Tool{
		...
		},
		{
			...
			Description: "Inspect a fresh Agent runtime for one-command support-session readiness: local rdev recovery, Go/Git checkout state, gateway URL candidates, agent_connection_runbook, gateway_candidate_preflight, configured RDEV_*_GATEWAY_URL fallbacks, platform helper asset availability, target bootstrap self-repair, and standard recovery actions. Agents should call this before improvising setup when rdev or gateway state is unclear, then use agent_connection_runbook, gateway_candidate_preflight, and the recommended gateway_url_candidates entry instead of asking humans to assemble gateway values or writing custom probes.",

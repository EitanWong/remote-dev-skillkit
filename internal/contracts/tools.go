...
func Tools() []Tool {
	...
	return []Tool{
		...
		},
		{
			...
			Description: "Create one short-lived browser handoff link for a Windows managed-host session. The native page adapts to browser language and opened system: Windows automatically consumes the one-time URL capability and directly displays a copyable PowerShell command; macOS, Linux, and other systems remain unclaimed and are told to open the same link on the target Windows host. The command fetches and hash-checks the scoped host binary. With visible UAC approval, it installs and starts the outbound Windows service. It returns handoff metadata plus user_summary and agent_next_action recovery fields.",

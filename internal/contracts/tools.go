...
func Tools() []Tool {
	...
	return []Tool{
		...
		},
		{
			...
			Description: "Create a short-lived browser handoff link for one Windows managed-host session. The native page adapts to browser language and Windows detection, claims a URL fragment proof once, downloads a double-click Windows launcher, and exposes a copy/paste PowerShell fallback with the same hash-checked host binary flow; it returns handoff metadata plus user_summary and agent_next_action recovery fields.",

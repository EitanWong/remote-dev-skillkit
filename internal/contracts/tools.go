...
func Tools() []Tool {
	...
	return []Tool{
		...
		},
		{
			...
			Description: "Run the standard Agent-native support-session smoke test against a connected host instead of hand-writing OS-specific job probes. The tool selects the single active host from ticket_code when possible, creates fixed scoped shell jobs for identity, fs.read, fs.write.scoped, and process.inspect, waits for terminal job states, summarizes artifacts, includes remote_control_entry and connection continuity, and tells Agents to keep the host connected unless the operator explicitly asks to disconnect.",

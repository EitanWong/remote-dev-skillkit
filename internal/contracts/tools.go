...
func Tools() []Tool {
	return []Tool{
		...
		},
		{
			...
			Description: "Single high-level Agent entry for 'connect a computer'. If a reachable or configured gateway exists, creates the attended-temporary session and returns user_handoff, connection_supervision, and status watcher in one payload. If no gateway is running, returns the standard visible foreground rdev support-session start command instead of failing or making the Agent choose lower-level handoff/create/start/status steps.",

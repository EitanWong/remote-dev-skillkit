...
func Tools() []Tool {
	return []Tool{
		...
		},
		{
			...
			Description: "Single high-level Agent entry for 'connect a computer'. If a reachable or configured gateway exists, creates the attended-temporary session and returns user_handoff, gateway_candidate_preflight, connection_supervision, and status watcher in one payload. If no gateway is running, returns cli_start_now_command for the standard visible foreground rdev support-session connect --start flow, plus the older support-session start fallback, instead of making the Agent choose lower-level handoff/create/start/status steps.",
		...
		},
		{
			...
			Description: "Inspect a fresh Agent runtime for one-command support-session readiness: local rdev recovery, Go/Git checkout state, gateway URL candidates, gateway_candidate_preflight, configured RDEV_*_GATEWAY_URL fallbacks, platform helper asset availability, target bootstrap self-repair, and standard recovery actions. Agents should call this before improvising setup when rdev or gateway state is unclear, then use gateway_candidate_preflight and the recommended gateway_url_candidates entry instead of asking humans to assemble gateway values or writing custom probes.",

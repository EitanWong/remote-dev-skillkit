...
func freshAgentSupportSessionChecks(input freshAgentSupportSessionCheckInput) []Check {
	...
	reachableCreateCommand := stringSliceFromAny(input.HandoffReachableGateway["cli_create_command"])
	...
	cliFollowUp := stringSliceFromAny(input.CreatedSession["cli_follow_up"])
	...
	stableFallbackCreateCommand := stringSliceFromAny(stableFallbackHandoff["cli_create_command"])
	...
	checks := []Check{
		...
		{Name: "connect_with_gateway_has_runner_recommendation", Passed: stringFromAny(connectRunnerRecommendation["schema_version"]) == supportsession.ConnectionEntryRunnerRecommendationSchemaVersion && stringFromAny(connectRunnerRecommendation["standard_interface"]) == "cli-only" && stringFromAny(connectRunnerRecommendation["standard_tool"]) == "" && strings.TrimSpace(stringFromAny(connectRunnerRecommendation["invite_json"])) != "", Detail: stringFromAny(connectRunnerRecommendation["target_os"])},
		...
		{Name: "handoff_with_gateway_selects_cli_create", Passed: input.HandoffReachableGateway["selected_path"] == "create-with-reachable-gateway" && input.HandoffReachableGateway["mcp_next_tool"] == "" && input.HandoffReachableGateway["next_interface"] == "cli-only", Detail: stringFromAny(input.HandoffReachableGateway["selected_path"])},
		{Name: "create_command_includes_gateway_and_waitable_target", Passed: containsAllStrings(reachableCreateCommand, "support-session", "create", "--gateway-url", "--target", "auto"), Detail: strings.Join(reachableCreateCommand, " ")},
		...
		{Name: "created_session_has_waiting_cli_followup", Passed: containsAllStrings(cliFollowUp, "support-session", "status", "--wait"), Detail: strings.Join(cliFollowUp, " ")},
		...
		{Name: "created_session_has_connection_supervision", Passed: stringFromAny(supervision["schema_version"]) == supportsession.ConnectionSupervisionSchemaVersion && !boolFromAny(mapFromAny(supervision["mcp_watch_call"])["available"]) && stringFromAny(mapFromAny(supervision["mcp_watch_call"])["tool"]) == "" && containsAllStrings(stringSliceFromAny(supervision["cli_watch_command"]), "support-session", "status", "--wait") && strings.Contains(stringFromAny(supervision["connected_report_rule"]), "connected_next_steps.user_report"), Detail: stringFromAny(supervision["upgrade_reason"])},
		...
		{Name: "started_payload_has_top_level_agent_runbook", Passed: stringFromAny(startedRunbook["schema_version"]) == supportsession.AgentConnectionRunbookSchemaVersion && stringFromAny(mapFromAny(startedRunbook["watch"])["interface"]) == "cli-only" && containsAllStrings(stringSliceFromAny(mapFromAny(startedRunbook["watch"])["cli_command"]), "support-session", "status", "--wait"), Detail: stringFromAny(startedRunbook["phase"])},
		...
		{Name: "stable_fallback_handoff_uses_configured_gateway", Passed: stringFromAny(stableFallbackHandoff["selected_path"]) == "create-with-reachable-gateway" && containsAllStrings(stableFallbackCreateCommand, "--gateway-url", "https://relay.example.test/rdev"), Detail: strings.Join(stableFallbackCreateCommand, " ")},

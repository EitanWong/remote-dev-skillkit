...
// workspaces, what authorizations are required, and what evidence must be collected.
type Package struct {
...
func Build(opts Options) (Package, error) {
	...
	pkg := Package{
		...
		AgentRules: []string{
			...
			"Confirm workspace stop and cleanup evidence before marking the task complete.",
		},
	}
...
func Verify(path string) (Verification, error) {
	...
	add("authorization_boundaries_declared", len(pkg.AuthorizationRequired) >= 4, fmt.Sprintf("%d", len(pkg.AuthorizationRequired)))
...
func packageChecks(pkg Package) []Check {
	return []Check{
		...
		{Name: "authorization_boundaries_declared", Passed: len(pkg.AuthorizationRequired) >= 4, Detail: fmt.Sprintf("%d", len(pkg.AuthorizationRequired))},
...
port-opening inside devcontainers require explicit operator authorization.

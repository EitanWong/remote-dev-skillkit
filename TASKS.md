...
- [x] Add high-level fresh-Agent `rdev support-session connect` / `rdev.support_session.connect` so Agents start with one "connect a computer" entry that either returns a ready human handoff or the standard visible foreground start command.

...
- [x] Add standard acceptance evidence scaffolding with `rdev.acceptance-evidence-scaffold.v1`, `rdev acceptance scaffold-evidence`, and MCP tool `rdev.acceptance.scaffold_evidence` so Agents create hosted-provider and relay/connectivity evidence directories, checklists, and package/verify commands from machine-readable plans instead of inventing file layouts.

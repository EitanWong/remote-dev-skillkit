...
- [ ] Run real fresh-agent Codex/Claude Code/Hermes/OpenClaw/OpenCode acceptance to confirm the Agent calls `rdev.support_session.create` when a gateway is reachable, falls back to `rdev.support_session.plan` only to prepare the gateway, and never invents gateway/bootstrap code.

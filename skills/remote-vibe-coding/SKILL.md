...
- Before trusting a managed or temporary host registration that includes an enrollment certificate, verify it with MCP tool `rdev.enrollment.verify_certificate` or CLI `rdev enrollment verify-certificate`; include `revocations_json` / `revocations_artifact_id` or `--revocations` when a signed `rdev.host-enrollment-revocations.v1` list is available. Certificate and revocation verification are read-only and never grant host access by themselves.

...
- For new adapters, start with MCP tools `rdev.adapter.verify_lifecycle` and `rdev.adapter.verify_result`, CLI commands `rdev adapter verify-lifecycle` and `rdev adapter verify-result`, or `pkg/adapterkit` conformance before exposing the adapter to agents; shell, PowerShell, and Codex are the reference fixtures for result artifacts.

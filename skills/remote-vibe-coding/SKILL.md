...
- For new adapters, start with MCP tool `rdev.adapter.verify_result`, `rdev adapter verify-result`, or `pkg/adapterkit` result-artifact conformance before exposing the adapter to agents; shell, PowerShell, and Codex are the reference fixtures.

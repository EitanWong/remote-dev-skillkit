...
- macOS Keychain and Windows DPAPI protected-store references for managed host identity and trust bundle persistence via `--identity-store keychain:<service>/<account>` / `--trust-store keychain:<service>/<account>` on macOS and `--identity-store dpapi:<service>/<account>` / `--trust-store dpapi:<service>/<account>` on Windows, preserving the same identity fingerprint, trust-bundle sequence, rollback checks, and host-bound update rules as file-backed development stores.
...
- Linux libsecret/keyctl managed host identity and trust storage beyond the current macOS Keychain, Windows DPAPI, and file-backed store paths.
- Artifact streaming.

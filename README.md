...
- Full production host enrollment authority lifecycle beyond the current local certificate, dev hosted issuance primitive, optional dev issuer bearer token, local renewal, signed empty revocation baseline, and dev revocation-list distribution primitives: operator roles, CA/key custody, production operator identity, renewal automation, authenticated hosted revocation refresh, and emergency drills.

...
kept local until an operator explicitly authorizes external GitHub mutations.
...
should be created until an operator explicitly authorizes that external change.
...
| Security Gates | `kind:security` plus authorization, trust, bootstrap, and persistence work |
...
|---|---|---|
| `v0.1 Local Safety Kernel` | signed local task execution, host-side verification, interrupts, evidence, audit, Skillkit export | substantially implemented locally; keep open only for release-note/evidence cleanup |
| `v0.2 Temporary Windows Host` | one visible verified Windows command starts an attended outbound host | clean Windows VM joins, verifies signed artifacts, enforces interrupts, revokes, leaves no persistence |
| `v0.3 Managed Mac Coding` | operator-owned managed Mac runs Codex tasks safely | service-backed LaunchAgent transcript proves reconnect, locked worktree, diff/test evidence, interrupt gates |
...
| `area:policy` | `d62728` | policy decisions, session interrupts, denial explanations |
| `area:evidence` | `9467bd` | artifacts, session evidence, audit export, redaction |
...
- host-side structured denials and interrupt-required artifacts;
- gateway-backed session evidence export;
...
- HTTPS long-poll session event transport prototype;
...
- shared risky-action interrupt preflight for shell, Codex, Claude Code, and acpx;
...
2. **Build Windows foreground temporary host no-persistence acceptance**
   ...
   - Acceptance:
     ...
     - host is foreground, outbound-only, revocable, and interrupt-gated;
...
3. **Add production WSS host channel with authenticated fallback**
   ...
   - Acceptance:
     - authenticated WSS supports interactive task status and artifact events;
...
6. **Harden ACP/acpx adapter behind the safety kernel**
   ...
   - Acceptance:
     - adapter runs only after signed envelope, host validation, workspace lock, and interrupt preflight;
...
8. **Package public Skillkit install paths for Codex Claude Code OpenCode Hermes and generic MCP**
   ...
   - Acceptance:
     ...
     - a self-host user can create a ticket, run a task, and export evidence without Hermes-specific assumptions.

...
- [ ] Smoke `rdev enrollment sign-certificate`, `rdev enrollment issue-certificate --gateway ... --root-public-key ...`, token-protected issuance with `--enrollment-issuer-token-file` / `--issuer-token-file`, `rdev enrollment verify-certificate`, `rdev enrollment init-revocations`, local `rdev enrollment renew-certificate --revocations`, hosted `rdev enrollment renew-certificate --gateway ... --root-public-key ...`, `rdev host serve --renew-enrollment-certificate --enrollment-root-public-key ...`, `rdev enrollment revoke-certificate --current`, `rdev enrollment verify-revocations`, `POST /v1/enrollment/certificates`, `POST /v1/enrollment/certificates/renew`, `GET /v1/enrollment/revocations` with both empty and non-empty lists, token-protected `GET /v1/enrollment/revocations`, `rdev enrollment fetch-revocations --issuer-token-file ...`, `rdev enrollment verify-certificate --revocations`, MCP tool `rdev.enrollment.verify_certificate` with revocation-list input, `rdev gateway serve --dev --enrollment-key ... --enrollment-revocations ...`, `rdev gateway serve --dev --enrollment-root-public-key ... --enrollment-revocations ...`, `rdev host serve --enrollment-certificate ...`, and `rdev host serve --fetch-enrollment-revocations --enrollment-root-public-key ... --enrollment-issuer-token-file ...` pre-registration rejection before making enrollment-certificate or revocation support claims.

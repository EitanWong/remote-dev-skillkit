...
- [ ] Smoke `rdev operator-auth verify-hosted`, `rdev operator-auth verify-oidc-jwks`, `rdev gateway storage verify`, `rdev acceptance package-hosted-provider-runtime`, `rdev acceptance verify-hosted-provider-runtime-package`, `rdev enrollment lifecycle key-custody`, `rdev enrollment lifecycle fleet-renewal-plan`, and `rdev enrollment lifecycle emergency-drill` before making hosted auth/storage or enrollment lifecycle claims.

...
- [ ] Confirm adapter scaffolds, lifecycle manifests, and built-in adapter result artifacts pass `pkg/adapterkit`, `rdev adapter scaffold`, `rdev adapter verify-lifecycle`, `rdev adapter verify-result`, `rdev.adapter.verify_lifecycle`, and `rdev.adapter.verify_result` conformance tests.

...
- [ ] Scaffold hosted provider runtime evidence with `rdev acceptance scaffold-evidence --plan <hosted-provider>/runtime-evidence-plan.json --out <evidence-input>`; confirm `AGENT_CHECKLIST.md`, `scaffold-report.json`, package/verify commands, and `ready_for_packaging=false` are present before collecting real evidence.
- [ ] Package and verify hosted provider runtime acceptance evidence with `rdev acceptance package-hosted-provider-runtime` and `rdev acceptance verify-hosted-provider-runtime-package`; confirm storage/auth verification reports `ok=true`, backup/restore/retention evidence exists, role probes include authorized and denied decisions, failure-mode probes pass, and audit evidence is redacted.
...
- [ ] Scaffold relay/connectivity adapter evidence with `rdev acceptance scaffold-evidence --plan <relay-adapter>/acceptance-evidence-plan.json --out <evidence-input>`; use `--create-placeholders` only when the placeholders will be replaced by real redacted transcripts before packaging.

...
- [ ] Scaffold hosted provider runtime evidence with `rdev acceptance scaffold-evidence --plan <hosted-provider>/runtime-evidence-plan.json --out <evidence-input>`; confirm `AGENT_CHECKLIST.md`, `scaffold-report.json`, package/verify commands, and `ready_for_packaging=false` are present before collecting real evidence; after collecting evidence, run `rdev acceptance evidence-status --scaffold <evidence-input>` and require `ready_for_packaging=true` before packaging.

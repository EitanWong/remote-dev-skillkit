...
- Release candidate verification that checks staged or downloaded candidates, including SBOM coverage/hash consistency, before publication or installation.
...
- Real build artifact generation through `scripts/release/build-artifacts.sh`, producing target-specific binaries, `rdev.build-artifacts.v1`, per-artifact `cgo_enabled` metadata, SPDX 2.3 SBOM, and checksums before candidate packaging.

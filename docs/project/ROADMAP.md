...
The roadmap implements the canonical [Perfect Ending Solution](../architecture/PERFECT_ENDING_SOLUTION.md): consent-first enrollment, outbound-only transport, signed job envelopes, host-side policy enforcement, workspace locks, approval gates, audit evidence, release-bundle verification, and agent-native MCP/Skill packaging. Its `Converged Perfect Ending - 2026-06-30` section is the top decision lens; future architecture changes should patch that layer instead of adding another dated "final" layer. The `Final Operating Decision` compresses the end state into one signed control protocol, two host products, separated release/bootstrap/gateway/host trust authorities, explicit join/run/approve/prove paths, and replayable proof packages. `Final Closure Specification - 2026-06-30` and `Definitive Perfect-Ending Blueprint - 2026-06-30` remain the implementation-grade decision detail for product boundaries, topology, protocol objects, mode contracts, state machines, permission rings, host sovereignty, adapter lifecycle, reliability, security, acceptance gates, v1.0 proof packages, and close order. Older dated final/kernel/product sections remain supporting rationale. That file is the final architecture lock and delivery contract for the safety microkernel and replaceable adapters for shell, PowerShell, Git, Codex, Claude Code, ACP/acpx, browser, GUI, mesh, Coder, DevPod, and future runtimes. [Final Closure Blueprint](../architecture/FINAL_CLOSURE_BLUEPRINT.md) is the concise release-facing summary, while [Ultimate Closure Design](../architecture/ULTIMATE_CLOSURE_DESIGN.md), [Final System Design](../architecture/FINAL_SYSTEM_DESIGN.md), and [Perfect End-State Architecture](../architecture/PERFECT_END_STATE.md) remain supporting context.
...
- Optional bearer-token protection for the dev hosted enrollment issuance endpoint through `rdev gateway serve --dev --enrollment-issuer-token-file` and `rdev enrollment issue-certificate --issuer-token-file`, keeping the token out of command output while requiring `Authorization: Bearer ...` for certificate issuance.
...
- Host-side hosted enrollment renewal before registration through `rdev host serve --renew-enrollment-certificate --enrollment-root-public-key`, using the gateway/mTLS-aware host client, optional issuer-token file, near-expiry threshold checks, pinned-root hosted renewal verification, certificate-file replacement, and gateway rejection when the current certificate is listed in signed revocations.
...
- Optional bearer-token protection for dev hosted enrollment revocation refresh, reusing `--enrollment-issuer-token-file` on the gateway and `--issuer-token-file` / `--enrollment-issuer-token-file` on CLI and host fetch paths.

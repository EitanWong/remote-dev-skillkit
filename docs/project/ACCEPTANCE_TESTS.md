...
- host-side hosted revocation refresh checks through `rdev host serve --fetch-enrollment-revocations --enrollment-root-public-key`, including explicit flag requirements, signed list verification, local certificate signature verification against the pinned root, success summaries, and revoked-certificate rejection before registration is attempted;

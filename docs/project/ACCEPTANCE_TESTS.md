...
- public adapterkit lifecycle-manifest and result-artifact conformance through `pkg/adapterkit`, `rdev adapter verify-lifecycle`, `rdev adapter verify-result`, and MCP tools `rdev.adapter.verify_lifecycle` / `rdev.adapter.verify_result`, covering adapter phases, safety boundaries, cancellation, cleanup, result schemas, and built-in shell, PowerShell, and Codex artifacts for schema, timing, redaction metadata, command evidence, cancellation/timeout exclusivity, and secret-pattern rejection.

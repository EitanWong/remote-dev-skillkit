...
- managed Mac LaunchAgent acceptance planning through `rdev acceptance managed-mac-service`, covering checked plist generation, managed host arguments, launchctl command plan, service-backed acceptance command plan, evidence verification command plan, and uninstall guidance without auto-starting launchd.

## Next Automation Targets

- Real managed Mac LaunchAgent acceptance execution: review generated plan, install/start with launchctl, reconnect after reboot, run locked-worktree Codex, verify evidence, stop, and uninstall.

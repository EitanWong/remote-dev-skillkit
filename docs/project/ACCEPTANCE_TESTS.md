...
- Windows temporary acceptance planning through `rdev acceptance windows-temporary`, covering reviewed PowerShell launcher generation, bootstrap script hash capture, release manifest/verifier requirements, approval probes for package/elevation/service/GUI/credential operations, no-persistence inspection commands, and required evidence checklist without executing PowerShell.
...
- Windows temporary host acceptance execution: run the generated plan on a clean Windows VM, verify signed release artifacts, run visible foreground bootstrap, confirm outbound-only host loop, collect no-persistence inspection output, approval-required probes, and revocation transcript.
- Release transcript packaging: publish redacted acceptance command transcript, verified report JSON, and evidence bundle checksums for each release candidate.

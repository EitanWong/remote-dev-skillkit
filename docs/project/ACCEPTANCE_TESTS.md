...
- Relay adapter acceptance evidence planning, packaging, and verification through `rdev.relay-adapter-acceptance-evidence-plan.v1`, `acceptance-evidence-plan.json`, `rdev acceptance package-relay-adapter`, and `rdev acceptance verify-relay-adapter-package`, covering verified relay adapter package archival, standard `rdev connection-entry run --result-out` runner capture, standard `rdev connection-entry run --helper-transcript-out` helper transcript evidence, helper transcript redaction, gateway/host/connection status archival, audit archival, checksums, a standard connectivity adapter `selected_path` (`existing-frp-or-chisel-relay`, `existing-ssh-tunnel`, `existing-headscale-tailscale-mesh`, or `existing-wireguard-vpn`), and `connected=true`.
...
- post-release download evidence scaffolding and readiness status through `rdev acceptance scaffold-post-release-download`, `rdev acceptance post-release-evidence-status`, and MCP tools, covering stable platform/Skillkit evidence directories, placeholder opt-in, missing/empty/placeholder detection, a fail-closed `ready_for_packaging=true` gate before `package-post-release-download`, plus package/verify rejection if platform, Skillkit, or post-release verification placeholder evidence reaches the archive path.
...
- Hosted provider runtime acceptance execution: run a hosted gateway with a verified provider package, capture gateway startup, `rdev gateway storage verify`, `rdev operator-auth verify-hosted`, `rdev operator-auth verify-oidc-jwks`, `rdev operator-auth verify-saml`, or equivalent hosted auth verification, backup, restore, retention, role-mapping authorization probes, failure-mode probes, and audit transcript, then package it with `rdev acceptance package-hosted-provider-runtime` and verify it with `rdev acceptance verify-hosted-provider-runtime-package`.

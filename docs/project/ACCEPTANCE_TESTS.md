...
- Hosted provider package generation and verification through `rdev hosted-provider package` and `rdev hosted-provider verify`, including schema checks, storage/auth provider declarations, gateway argument templates, environment declarations, checksums/sizes, unlisted-file rejection, `external_mutation=false`, and no-private-surface checks.
- Hosted provider runtime acceptance evidence packaging and verification through `rdev acceptance package-hosted-provider-runtime` and `rdev acceptance verify-hosted-provider-runtime-package`, covering verified hosted provider package archival, gateway startup evidence, storage verification, hosted auth verification, backup evidence, restore evidence, retention policy evidence, role-mapping probes with both authorized and denied decisions, failure-mode probes, audit archival, redaction, checksums, and no-private-surface checks. Built-in `file` plus `hosted-ed25519-jwt` evidence can satisfy a single-node smoke gate; durable external provider claims still require deployed provider evidence.
- Relay adapter package generation and verification through `rdev relay-adapter package` and `rdev relay-adapter verify`, including Chisel/frpc adapter schemas, standard `RDEV_RELAY_*` runner metadata, safe helper argv, safe dependency install actions, checksums/sizes, unlisted-file rejection, `external_mutation=false`, and no-private-surface checks.
- Relay adapter acceptance evidence packaging and verification through `rdev acceptance package-relay-adapter` and `rdev acceptance verify-relay-adapter-package`, covering verified relay adapter package archival, Connection Entry runner result archival, helper transcript redaction, gateway/host/connection status archival, audit archival, checksums, `selected_path=existing-frp-or-chisel-relay`, and `connected=true`.
- Release candidate packaging through `rdev release prepare-candidate`, covering staged built artifacts, signed artifact manifests, signed release bundle verification, Skillkit export/verify, SPDX 2.3 SBOM, local provenance, checksums, `connection-entry-release.zip`, and `rdev.release-candidate.v1` summary generation without publishing to GitHub or leaking local output paths.
- release candidate verification through `rdev release verify-candidate`, covering relocated candidates, summary schema, root public key, checksums, signed release bundle verification, required artifacts, Skillkit verification, SBOM coverage, provenance coverage, Connection Entry release archive schema/checksums/no-private-parameter policy, listed file hashes/sizes, and unlisted-file rejection.
...
- Restrictive-network relay acceptance execution: run a verified Chisel/frpc relay adapter package against a deployed relay endpoint on clean Windows/macOS/Linux targets, capture the Connection Entry runner result, helper transcript, gateway status, host status, connection status, and audit transcript, then package it with `rdev acceptance package-relay-adapter` and verify it with `rdev acceptance verify-relay-adapter-package`.
- Hosted provider runtime acceptance execution: run a hosted gateway with a verified provider package, capture gateway startup, `rdev gateway storage verify`, `rdev operator-auth verify-hosted` or equivalent hosted auth verification, backup, restore, retention, role-mapping authorization probes, failure-mode probes, and audit transcript, then package it with `rdev acceptance package-hosted-provider-runtime` and verify it with `rdev acceptance verify-hosted-provider-runtime-package`.

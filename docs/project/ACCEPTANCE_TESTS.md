...
- Connectivity adapter package generation and verification through `rdev relay-adapter package` and `rdev relay-adapter verify`, including Chisel/frpc `RDEV_RELAY_*`, SSH tunnel `RDEV_SSH_*`, headscale/Tailscale-compatible mesh `RDEV_MESH_*`, and WireGuard `RDEV_VPN_*` adapter schemas, safe helper argv, safe dependency or manual-review install actions, checksums/sizes, unlisted-file rejection, `external_mutation=false`, and no-private-surface checks.
...
- Restrictive-network relay acceptance execution: run a verified Chisel/frpc, SSH tunnel, headscale/Tailscale-compatible mesh, or WireGuard connectivity adapter package against a deployed or already configured route on clean Windows/macOS/Linux targets, capture the Connection Entry runner result, helper transcript, gateway status, host status, connection status, and audit transcript, then package it with `rdev acceptance package-relay-adapter` and verify it with `rdev acceptance verify-relay-adapter-package`.

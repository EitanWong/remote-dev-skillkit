...
terminal MCP server. Its job is to let agents cause useful work only through visible
...
2. The gateway signs only bounded, host-specific, expiring job envelopes.
3. The host independently verifies identity, signature, expiry, nonce, policy,
   capabilities, workspace scope, approvals, and locks before execution.
4. Adapters execute work but never own authorization, persistence, approval, or trust.
...
10. Tickets, hosts, jobs, approvals, keys, and sessions can be revoked and audited.

## Final Planes

| Plane | Owns | Must not own |
|---|---|---|
| Agent interface | Skills, MCP tools, typed requests, policy explanations, evidence review | host credentials, approval authority, raw default shells |
...
| Host sovereignty | local identity, trust bundle, nonce and approval stores, local policy, locks, adapter runner, stop control | broader authority than the gateway and local policy grant |
| Adapter execution | shell, PowerShell, Git, Codex, Claude Code, ACP, browser, GUI, mesh, Coder, DevPod work | authorization, persistence, approval, identity, release trust |
| Review and release | evidence bundles, audit export, release manifests, checksums, signatures, rollback | runtime job authorization |
...
approval for push, merge, deploy, publish, paid actions, credential changes, GUI
...
- jobs, signed envelopes, leases, nonces, and cancellations;
- approval requests, approval tokens, and host-side approval consumption;
- denials, approval-required pauses, adapter results, and evidence bundles;
...
- job creation, claim, completion, cancellation, and artifact upload are idempotent;
...
- terminal state and evidence are separate: a canceled job remains `canceled`, but the
...
escape rejection, approval pauses before side effects, cancellation hooks, redaction,
...
- Mesh tools such as Tailscale/headscale are optional owned-host transports, not job
  authorization.
...
- agent self-approval for dangerous operations;
- automatic privilege escalation or OS security weakening;
- silent persistence on third-party machines;
- GUI control without local/operator approval and audit;
...
4. Evidence bundles and hash-chained audit exports let another human or agent reconstruct
   what happened.

...
`gateway_candidate_preflight`, and `standard_recovery` fields instead of writing custom bootstrap, approval, or
relay glue. Read `gateway_candidate_preflight` before asking network questions or writing probes, then use the recommended `gateway_url_candidates` entry for target-side

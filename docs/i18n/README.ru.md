...
|---|---|
| Skills, MCP tools и адаптеры files/desktop/tasks | Видимость, authorization, revocation и audit |
| Подписанные tasks с понятными capabilities | Локальную host policy и security boundaries |
| Artifacts и session evidence | Контроль над тем, что запускается и когда останавливается |

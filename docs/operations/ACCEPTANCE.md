...
rdev connection-entry run \
  ...
  --helper-transcript-out helper-transcript.txt
```

Use `runner-result.json` as the `--runner-result` input and
`helper-transcript.txt` as the `--helper-transcript` input for
...
results, and manual-action requirements. The helper transcript is standard
plain-text evidence generated by the runner from dependency install,
